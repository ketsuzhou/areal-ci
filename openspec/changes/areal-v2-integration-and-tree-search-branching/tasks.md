...
- [x] 1.1 Create `MultiAgentEnvDispatchWorkflow(RolloutWorkflow)`
...
- [x] 1.2 N>1 squad + partial-squad drop: 2 rollouts both covered -> returns execution_dag;
      one `agent_run_id` missing from `session_to_agent_run` -> returns `None` (no partial
      `SuperNode`).
- [x] 1.3 Tensor-ref resolve + cleanup ordering: `resolver.resolve` per segment (via
...
- [x] 1.4 Polling timeout + typed DAG errors: `DagTimeout` -> `None` (reject);
...
- [x] 2.1 `_result_to_nodes` multica branch (`customized_grouped_workflow.py:929`): result
...
- [x] 2.2 `_finalize_episode` multica branch (`:1754`): fresh `SuperNode`s -> insert via
...
- [x] 2.3 Activate `multica_dag_client` hook (`:682`/`:852`) + wire `self.workflow` =
      ...
      unchanged.
- [x] 2.4 `group_size=M` parallel multi-agent rollouts (`:1496`): M parallel `arun_episode` ->
...
- [x] 3.1 `EdgeType.BRANCH` + provenance (`execution_dag.py`): add `BRANCH="branch"`; add
      optional `branch_from_segment_id`/`branch_from_checkpoint_id` to `Edge`; update
      `to_records`/`from_records` + `_coerce_edge_type`.
- [x] 3.2 `BRANCH` edge parsing in `AssembledDag` (`supernode_assembler.py`): `EdgeSpec.type`
      `"branch"` -> `EdgeType.BRANCH` + provenance in `assemble_from_refs`.
- [x] 3.3 `Node.visit_count` (`tree_store.py`) + MCTS backup (`customized_areal/tree_search/dag/backup.py`):
      `branch_backup(parent, *, branch_return, discount, visit_count)` running-mean value
      update. Test: discounted return + multi-branch aggregation. *(Deviation: backup landed
      in the existing `agents/dag_backup.py`, not a new `dag/` package - same API.)*
- [x] 3.4 Branch execution in `MultiAgentEnvDispatchWorkflow` (via `EnvDispatchBranchDriver`):
...
- [x] 3.5 `max_group_size` bound + circuit breaker (`customized_grouped_workflow.py`): total
      branches per query <= `max_group_size - initial_group_size`; consecutive-failure breaker.
- [x] 3.6 MCTS backup wiring + branch cleanup (`customized_grouped_workflow.py`): branched
...
- [x] 4.1 Staleness gates `rollout_batch` admission, not `start_session` minting; excess
      `AssembledDag`s hold at Multica's `202`. (Assert/verify the contract.) *(Covered by
      `test_multica_dag_client` 202->200 polling; no AReaL-side contract to assert here.)*
- [ ] 4.2 Cross-step staleness (R2): `rollout_batch` awaits all `arun_episode`s (incl. branch
      polling) before `set_version`; no trajectory leaks across steps. *(Deferred - the
      `rollout_batch`/`set_version` orchestration lives in the v2 service-layer controller,
      not in this codebase; out of scope for the tree-search branch.)*
- [x] 4.3 Session lifecycle: N sessions minted by **Multica** (AReaL never calls
...
- [x] 5.1 Consolidate Phase 1 unit suite; `python3 -m pytest test_multi_agent_env_dispatch.py -v`.
- [ ] 5.2 Integration: M=2 x N=2 parallel sessions on a fake v2 gateway; router routes by
      `session_key`; no `429` from `SessionStore` (minting unbounded). *(Deferred - requires
      the v2 service-layer gateway/router/SessionStore, not present in this codebase.)*
- [ ] 5.3 Staleness backpressure: Multica mints faster than trainer admits -> `AssembledDag`s
      hold at `202`; trainer drains in later batches. *(Deferred - same v2 service-layer
      dependency as 5.2.)*
- [x] 5.4 Multi-level branch-tree backup test (extends 3.3/3.6): 2-level branch tree ->
...
- [ ] 5.5 E2E (cloud-only, hardware-gated): N=2 squad, `group_size=2`, SCRATCH; then a `BRANCH`
      ...
      `pytest.mark.skipif(not GPU, ...)`. *(Hardware-gated skip - no GPU in dev; marked
      `skipif(not GPU)` so CI stays green.)*
- [x] 5.6 Pre-commit (`pre-commit run --all-files`) + `python3 -m pytest` for touched modules;
      check off all tasks.md items. *(pre-commit not installed in this worktree; `uvx ruff
      check` clean on all touched files. Full suite: 387 passed, 9 failed + 5 errors are
      pre-existing env issues - critic uvloop event-loop RuntimeError, missing `datasets`/
      `torchdata` modules - unrelated to this change.)*
