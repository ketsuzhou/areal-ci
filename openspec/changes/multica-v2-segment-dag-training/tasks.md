...
- [x] 0.2 Commit: `gov(multica-v2-segment-dag): supersede sub-project-g interaction-dag`.

## 1. Investigation - v2 surface and Multica seams

- [x] 1.1 Confirm v2 `SessionData` (`areal/v2/inference_service/data_proxy/session.py`):
...
- [x] 1.2 Confirm v2 `/export_trajectories` (`data_proxy/app.py`): current multi-session
  merge + `RTensor.remotize` path; what it takes to export a single `trajectory_id` with
  `remove_session=False`.
- [x] 1.3 Confirm v2 data_proxy `/data/<shard_id>`, `/data/batch`, `/data/clear` surface
  (add what is missing).
- [x] 1.4 Confirm v2 gateway routes (`gateway/app.py`) to add `/rl/close_segment` and
  per-trajectory export.
- [x] 1.5 Confirm Multica driving seam (proxy `/chat/completions` boundary) and where
  `close_segment` + export are called per communication event (delegation / mention /
  completion / squad briefing).
- [x] 1.6 Confirm `session_id <-> agent_run_id` capture at `/rl/start_session`
  (`server/internal/arealrl/client.go`) and the root-task completion signal that flips the DAG
  endpoint `202` -> `200`.
- [x] 1.7 Decide `close_segment` on empty active (error vs empty trajectory) and record in
  design.
- [x] 1.8 Commit: `docs(v2-segment-dag): T1 v2 + multica seam confirmation`.

## 2. Migration + DB queries (Multica)

- [x] 2.1 Add migration: `interaction_dag_segment` (segment_id, project_id, agent_run_id,
...
- [x] 2.2 Generate / add DB query files: `CreateSegment`, `CloseSegment` (set
...
- [x] 2.3 Ensure `DELETE /api/v1/env-dispatch/{projectID}` cascades to the new tables.
- [x] 2.4 Commit: `feat(v2-segment-dag): migration + queries for segments edges snapshots`.
...
- [x] 4.1 Failing tests: `/export_trajectories` by `trajectory_id` with
  `remove_session=False` returns `RTensor` refs for that segment and leaves other
  trajectories ready.
- [x] 4.2 Failing tests: exported payload is refs-only - no message text (only `input_ids`,
  `loss_mask`, `logprobs`, `versions`, `attention_mask`).
- [x] 4.3 Failing tests: exporting an unknown `trajectory_id` returns a typed error.
- [x] 4.4 Failing tests: `/data/<shard_id>` and `/data/batch` resolve refs to tensor bytes;
  `DELETE /data/clear` frees shards.
- [x] 4.5 Implement per-trajectory export + `/data/*` resolve + `/data/clear` (add missing
  endpoints).
- [x] 4.6 Commit: `feat(v2-segment-dag): per-segment tensor-ref export + data resolve`.
...
- [x] 5.1 Failing tests: at a delegation event Multica calls `close_segment` + export on the
  parent run, records a segment with `trajectory_id` + `tensor_ref` + `closing_event`, opens
  a child segment, records a `DELEGATION` edge.
- [x] 5.2 Failing tests: mention records a `MENTION` edge without closing a segment.
- [x] 5.3 Failing tests: completion calls `close_segment` + export on the child run, records
  a `COMPLETION` edge.
- [x] 5.4 Failing tests: squad briefing closes a segment via `close_segment` + export.
- [x] 5.5 Failing tests: leaf run (no communication event) yields one leaf segment with
  `closing_event = None`.
- [x] 5.6 Failing tests: concurrent fan-out delegation produces multiple `DELEGATION` edges
  and a deterministic, acyclic segment set.
- [x] 5.7 Failing tests: `RecordSessionAgentRun` captures `session_id <-> agent_run_id`.
- [x] 5.8 Failing tests: a recording error is logged and the run continues (best-effort).
- [x] 5.9 Implement `InteractionDAGService` (record turn, close segment + export on event,
  add edge, capture ref-only snapshot) behind a feature flag.
- [x] 5.10 Wire hooks from task / mention / completion / squad-briefing seams for trained
  rollouts only.
- [x] 5.11 Commit: `feat(v2-segment-dag): incremental segment + edge recording`.
...
- [x] 6.1 Failing tests: `AssembleAssembledDag(project_id)` returns `segments`, `edges`,
...
- [x] 6.2 Failing tests: edges carry `src` / `dst` / `type` = `delegation` / `mention` /
  `completion`; assembled DAG is acyclic.
- [x] 6.3 Failing tests: `GET /api/v1/env-dispatch/{projectID}/dag` returns `202` in
  progress, `200` + `AssembledDag` done, `404` unknown project, `403` cross-workspace.
- [x] 6.4 Failing tests: incomplete/failed rollout yields a `failed` status, not a partial
  `AssembledDag`.
- [x] 6.5 Implement assembly from recorded rows + the polling handler.
- [x] 6.6 Commit: `feat(v2-segment-dag): assemble AssembledDag + polling endpoint`.
...
- [x] 7.1 Failing tests: `get_env_dispatch_dag(project_id)` polls until `200` and returns an
  `AssembledDag`; `202` retried with backoff; `404`/`403` raise typed errors.
- [x] 7.2 Failing tests: assembler resolves each segment's `tensor_ref` via `/data/*` to
  tensors and builds a `SuperNode(payload=tensors, metadata=segment)` - no turn-index slice.
- [x] 7.3 Failing tests: edges build an acyclic `ExecutionDAG` reconstructing
  delegation / mention / completion topology.
- [x] 7.4 Failing tests: a session whose segments do not densely cover the run raises
  `DAGError` (no partial DAG).
- [x] 7.5 Failing tests: a synthetic 3-segment planner -> worker -> synthesizer
  `AssembledDag` round-trips losslessly into the `ExecutionDAG`.
- [x] 7.6 Implement the adapter + the ref-resolve assembler path.
- [x] 7.7 Commit: `feat(v2-segment-dag): areal client + ref-resolve assembler`.
...
- [x] 8.1 Failing tests: the built DAG is consumed by a minimal training step using a
  placeholder reward (terminal `set_reward` or zero) - end-to-end path exercises without a
  judge.
- [x] 8.2 Failing tests: after training, `DELETE /data/clear` frees the resolved shards and
  the session is revoked once its last segment is consumed.
- [x] 8.3 Failing tests: refs are not cleared before training resolves them.
- [x] 8.4 Implement the minimal training hook + cleanup sequence.
- [x] 8.5 Commit: `feat(v2-segment-dag): minimal training plumbing + tensor lifecycle`.

## 9. Config + production wiring

- [x] 9.1 Add `INTERACTION_DAG_ENABLED` (default on for trained rollouts) and v2
  `close_segment` / per-segment export feature flags.
- [x] 9.2 Add polling config on the AReaL side (interval, timeout, backoff).
- [x] 9.3 Wire the recording service + v2 ops into construction behind the flags.
- [x] 9.4 Document the `AssembledDag` contract + v2 ops in
  `customized_areal/tree_search/agents/multica_environment_protocol.md`.
- [x] 9.5 Commit: `chore(v2-segment-dag): config + production wiring`.

## 10. Full regression + E2E + grep sweep

- [x] 10.1 Scoped Multica Go build/test/vet for touched packages; `gofmt -l` clean. *(recording-assembly verified 2026-07-10)*
- [x] 10.2 AReaL: `uv run pytest areal/v2/inference_service/tests/` and
  `uv run pytest customized_areal/tree_search/tests/ -k 'segment_dag or supernode or env_dispatch'`. *(31 passed 2026-07-10)*
- [x] 10.4 Verify env snapshots are refs-only (no sandbox pause/fork) - F-independence. *(design D3/D4)*
- [x] 10.5 grep sweep: `close_segment`, `AssembledDag`, `tensor_ref`, `v2-segment-dag`,
  `env-dispatch/{projectID}/dag` resolve to intended code only; no `start_turn_idx` /
  `end_turn_idx` remain in the new tables/code. *(recording-assembly)*
- [x] 10.7 Commit: `docs(v2-segment-dag): T10 full regression + E2E + grep sweep`. *(folded into recording-assembly verification commits)*
...
- 10.3 Cross-repo E2E if feasible: `mode=scratch` rollout with a 3-agent team ->
  ...
  *(skipped in build: services/GPU unavailable; to attempt in comet-verify)*
- 10.6 Final whole-branch review -> READY TO MERGE / NEEDS_CHANGES. *(verify-phase activity)*
