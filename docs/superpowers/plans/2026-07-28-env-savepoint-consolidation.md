...
- `sqlc` is **not installed in this devcontainer** (`which sqlc` → not found).
  `pkg/db/generated/env_checkpoint.sql.go` carries
  `// Code generated by sqlc ... sqlc v1.31.1`, so generation does work with the right
  toolchain.
- Some recent generated files are hand-written instead, carrying the marker
  `// Hand-written (sqlc generate is broken in this repo)` — e.g.
  `pkg/db/generated/interaction_dag.sql.go:357`,
  `pkg/db/generated/interaction_dag_diagnosis.sql.go:6`.
- **Rule for this change:** hand-write the `pkg/db/generated/*.sql.go` additions in
  exactly the style of the surrounding generated code (`const <lowerCamelName> = ...`,
  `type <Name>Params struct`, `func (q *Queries) <Name>(...)`), and add the same
  `// Hand-written (sqlc generate is broken in this repo)` marker comment.
  `go build ./...` must pass.

### Verification capability limits in this environment

Established by direct check, not assumption: `sqlc` is not installed, `psql` is not
installed, Docker is not available, and the `DATABASE_URL` configured in `multica/.env`
points at `localhost:5432`, which refuses connections. Installing Postgres was
explicitly declined for this change.

Consequences that every schema and query task below must respect:

- Generated sqlc code is hand-written (see the rule above), so nothing machine-checks
  the SQL text against the real schema.
- The `*_Integration` query tests this plan specifies are still **written**, and they
  ...
  them as passing.
- Therefore the acceptance evidence available in this environment is: `go build ./...`,
  `go vet ./...`, and the fake-injected unit tests. Migrations, unique-index behavior,
  cascade behavior, and the hand-written SQL remain **unverified until run against a
...
`CheckpointID pgtype.UUID`, and the two new `Queries` methods exist. If `sqlc` is
unavailable, hand-write these additions per the **sqlc regeneration** constraint above
and re-run `go build ./...`.
