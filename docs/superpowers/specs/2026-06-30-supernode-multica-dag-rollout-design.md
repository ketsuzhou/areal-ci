...
**Public API**:
- `insert_super_batch(supers: list[SuperNode], backup: bool = True)` replaces `insert_batch(list[Node])`. When `backup=True`, it runs one root-ward backup per episode: nodes carrying an `episode_id` are backed up from their episode terminal; nodes **without** an `episode_id` (multi-agent assembled SuperNodes, where credit flows along the cross-agent parent DAG, not a single episode) are backed up once per **leaf** (a node no other inserted node points to as a parent). The leaf's reward is read from `self._rewards[node_id]`, so **the verifier's `set_reward`/`_rewards` population must run before `insert_super_batch(backup=True)`** on assembled multi-agent SuperNodes (or use `backup=False` + explicit `backup_episode_terminal`).
- `get_super_node(uuid) -> SuperNode | None`.
- `get_node(node_id)` walks into a SuperNode's `nodes` via `_node_id_to_super`.
- `backup_episode_terminal(terminal_node_id, reward)` — root-ward DFS over `parent_node_id` + `extra_parent_node_ids` across SuperNode/agent boundaries (crediting every branch at a join, once per ancestor via a `visited` guard).
- `backup_path_returns(terminal_node_id, returns_by_node_id)` — same multi-parent DFS, applying per-node returns.
...
- **Node**: one assistant turn (`input_ids`, `loss_mask`, `logprobs`, etc.); torch-lazy after refactor. Causal linkage lives on `parent_node_id` (primary predecessor) and `extra_parent_node_ids` (additional predecessors at fan-in joins).
