...
**File:** `areal/experimental/inference_service/controller/controller.py`
**Method:** `RolloutControllerV2._resolve_workflow`

Three edits:

a. Extract `tree_search_config` from `workflow_kwargs` at method start (prevents
   leakage into InferenceServiceWorkflow / agent constructors).

b. Replace the `GroupedRolloutWorkflow` wrapping in the online branch with a call to
   a new `_wrap_grouped` helper that checks `tree_search_config.enabled`.
...
`TreeSearchGroupedRolloutWorkflow`; otherwise returns `GroupedRolloutWorkflow`.
Exact port of V1 lines 707-745.
...
`RolloutWorkflow.arun_episode()` returning `dict[str, InteractionWithTokenLogpReward] | None`.
The wrapping is transparent to `TreeSearchGroupedRolloutWorkflow`.
