    ...
    * ``G_latest^(n)(s_t)`` [return]: n-step return along the episode,
    ...
    def _is_new(self, node: Node) -> bool:
        ...
        if self.tree_store.latest_version < 0:
            return False
        return (
            int(getattr(node, "version_id", -1) or -1) == self.tree_store.latest_version
        )
    ...
    def _v_latest(self, node: Node, excluded_reward: float) -> float:
        ...
        """
        node_id = getattr(node, "node_id", None)
        if self.tree_store.latest_version >= 0 and node_id and not self._is_new(node):
    ...
    def compute(self, trajectories: list[Node]) -> None:
        ...
        for nodes in episodes.values():
            ...
            v_own = [self._blended_value(n, excluded[i]) for i, n in enumerate(ordered)]
            v_latest_vals = [
            ...
            for t, node in enumerate(ordered):
                ...
                g_latest = n_step_return(rewards, t, horizon, self.gamma, bootstrap)
