...
``max(3, max_group_size)`` consecutive failures. Failed episodes do not count
against the budget (they count against the circuit breaker), so both bounds
remain operative.
"""
...
async def test_budget_zero_prevents_branch_episodes(tmp_path):
    ...
    fake = _NoneWorkflow()
    wf = _make(
        tmp_path, initial_group_size=2, max_group_size=2, workflow=fake
    )
...
async def test_consecutive_failures_trip_circuit_breaker(tmp_path):
    ...
    # max(3, max_group_size) failures.
    fake = _NoneWorkflow()
    wf = _make(
        tmp_path, initial_group_size=2, max_group_size=4, workflow=fake
    )
    await wf._arun_episode_dynamic(engine=None, data={}, query_id="q1")
    # initial (2) + max_failed_additions (= max(3, 4) = 4) consecutive failures.
