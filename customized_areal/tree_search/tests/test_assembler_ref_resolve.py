...
def _dag_with_rewards() -> AssembledDag:
    """_dag() with one exact assistant-turn diagnosis reward on seg-1."""
    dag = _dag()
    dag.segments[0].assistant_turn_seqs = [1]
    dag.step_rewards = [
...
def test_assemble_from_refs_assigns_step_rewards_to_nodes_not_supernodes():
    dag = _dag_with_rewards()
    edag = SuperNodeAssembler().assemble_from_refs(dag, FakeResolver())
    scored = edag.get("seg-1").nodes[0]
    assert scored.episode_scores == {"trajectory:0": 8.0}
    assert scored.process_reward == pytest.approx(1.0)
    assert edag.get("seg-1").process_reward == 0.0
    assert edag.get("seg-2").process_reward == 0.0


def test_assemble_from_refs_keeps_supernode_process_reward_zero():
    dag = _dag_with_rewards()
    dag.score_max = 0
    edag = SuperNodeAssembler().assemble_from_refs(dag, FakeResolver())
    assert edag.get("seg-1").process_reward == 0.0
...
def test_assemble_from_refs_ignores_unmatched_step_rewards_without_creating_nodes():
    dag = _dag_with_rewards()
    dag.step_rewards.append(
        StepReward(segment_id="seg-ghost", seq=1, score=9, rationale="x")
    )
    edag = SuperNodeAssembler().assemble_from_refs(dag, FakeResolver())
    assert edag.get("seg-1").nodes[0].process_reward == pytest.approx(1.0)
