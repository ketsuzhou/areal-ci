...
decision 3). Each :class:`SuperNode` carries the ``session_id`` assigned at
...
def test_session_map_returns_node_to_session() -> None:
    dag = ExecutionDAG()
    dag.add_event(_run("A", session_id="sess-A"))
    dag.add_event(_run("B", session_id="sess-B"))
...
def test_session_map_includes_none_for_session_less_node() -> None:
    dag = ExecutionDAG()
    dag.add_event(_run("A", session_id="sess-A"))
    dag.add_event(_run("B"))  # no session started yet
