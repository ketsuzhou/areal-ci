"""Branch-point selection over the canonical SuperNode sequence (ported criterion).

Torch-free, I/O-free, pure. Ports
``core/customized_grouped_workflow.py::select_branch_candidate`` (critic
TD-error gate + max-entropy ranking) onto the SuperNode/DAG representation,
emitting one :class:`BranchPoint` per ``task_id`` lane. The emitted
``(task_id, seq)`` pairs are the key accepted by
``event_codec.replay_prefix_for``.
...
class BranchPoint:
    ...
    bypassed because the SuperNode had no critic value). ``entropy`` is the
...
def _entropy(event: SuperNode) -> float:
...
def lane_successor_value(
    event: SuperNode, lane_children: Mapping[str, SuperNode]
) -> tuple[float, float]:
...
def td_error(
    event: SuperNode, r_t: float, v_next: float, *, gamma: float
) -> float | None:
...
def _lane_children(events: Sequence[SuperNode]) -> dict[str, SuperNode]:
...
def select_branch_points(
    ...
    *,
    ...
    Raises ``DAGError`` (via ``supernodes_to_dag``) on a malformed SuperNode log.
    """
    events = list(events)
    if not events:
        return []
    ...
    for task_id in sorted(lanes):
        survivors: list[tuple[SuperNode, float | None]] = []
