...
- [x] Task 4: Trigger at collaborative-task completion (Go) - COMPLETE (multica dev 11d07dbc0..e7c38ff26, single commit e7c38ff26, SDD task review APPROVED). Orchestrator-IMPLEMENTED DIRECTLY (no implementer subagent - 2 consecutive false-GREENs established direct-impl as the mode; reviewers remain reliable). Deliverables: Diagnoser interface (Diagnose(ctx,projectID) ([]StepReward,error)) + Diagnosis Diagnoser field on TrainingSessionDeps (nil-safe, mirrors DAG) + var _ Diagnoser=(*DiagnosisAgentRunner)(nil) compile-check; maybeDiagnoseProject helper (gated: deps==nil || Diagnosis==nil || DAG==nil || !DAG.Enabled() -> return; root-task gate task.AgentID==dispatch.TrainAgentID both Valid -> return otherwise; calls Diagnose then DAG.RecordStepRewards; soft-fail: Diagnose err slog.Warn+return no rewards, RecordStepRewards err slog.Warn+proceed; void fn, never fatal); wired into RouteTerminalTrainingTask AFTER maybeTriggerCheckpoint + BEFORE the !dispatch.CriticAgentID.Valid branch (so diagnosis+RecordStepRewards land before SetReward/EndSession in BOTH no-critic-close and critic-deferred-close paths). Deviation from plan (justified): Diagnoser interface instead of concrete *DiagnosisAgentRunner - matches codebase deps-as-interfaces pattern (arealSessionCloser etc.) + testability (fakeDiagnoser). Controller-verified: go build ./internal/service/ ./internal/handler/ ./cmd/server/ exit 0 (cmd/server webpush did NOT reproduce here); go vet exit 0; TestMaybeDiagnoseProject_RootTask_FiresAndRecords PASS, TestMaybeDiagnoseProject_GatingOff 4/4 (diagnosis_nil/dag_disabled/dag_nil/non_root_task), TestMaybeDiagnoseProject_SoftFail PASS, TestDiagnosisBeforeCloseHook_Ordering PASS ([Diagnose,RecordStepRewards,SetReward,EndSession] via shared order slice); full service suite 246 PASS/0 FAIL/15 SKIP. SDD task reviewer (sonnet a2520d70): VERDICT APPROVED - all spec contracts PASS (file:line evidence); no production defects; independent re-verification build/vet/tests all exit 0, exactly 3 files. CORRECTION to Task 5 ledger: 4.4 "202 while diagnosis runs" does NOT fall out naturally - CompleteAgentTask persists terminal status BEFORE RouteTerminalTrainingTask, so /dag sees terminal (200, not 202) during the synchronous diagnosis. Task 4 guarantees step_rewards written before the close hook, NOT strict 202-during-diagnosis. DEFERRED gaps (NOT Task 4): (1) Diagnose placeholder prompt (--no-tools, Task 1) - Task 3's tools NOT yet wired into runner; rich-prompt assembly is a separate piece (no explicit task - flag for resolution, likely fold into Task 7 integration); (2) strict 4.4 /dag 202-during-diagnosis (needs a diagnosis-in-progress flag); (3) 3 test-strengthening notes (Important: ordering test calls helpers manually not via real RouteTerminalTrainingTask - hermetic RTT test blocked by concrete s.Queries.GetIssue, RTT harness is DB-backed setupRetryTestDB; Minor: soft-fail+close-hook-fires combo untested; Minor: RecordStepRewards-error path untested) -> defer to final review / Task 7 integration (7.1 exercises real RTT path end-to-end). openspec group 3: 3.1-3.3,3.5,3.6 checked; 3.4 unchecked (views whole DAG needs rich-prompt) at Task 4 time; CLOSED by 5ed9f358a (rich-prompt Diagnose) before Task 6 - see Task 6 entry.
- [x] Task 6: Config + flags (Go) - COMPLETE (multica dev 5ed9f358a..afbb5e601, single commit afbb5e601, controller-direct-impl TDD RED-first). Orchestrator-IMPLEMENTED DIRECTLY (direct-impl mode per Tasks 4/5; quota reset 2026-07-13 but 2 prior false-GREENs keep direct as the mode). Deliverables: TrainingConfig +5 fields (DiagnosisAgentEnabled/Path/Model/Timeout/ScoreMax); LoadTrainingConfig parses DIAGNOSIS_AGENT_ENABLED (default off), _PATH, _MODEL, _TIMEOUT_SECONDS (default 60s), _SCORE_MAX (default 10) mirroring the INTERACTION_DAG_ENABLED parse pattern - invalid numeric env -> slog.Warn + default (no daemon failure); buildDiagnoser(cfg,q) constructs NewDiagnosisAgentRunner(Provider:"pi", DAGStore:q, MessageStore:q) ONLY when enabled + only in the full bridge+DAG production path (q!=nil && bridge configured); *db.Queries satisfies both InteractionDAGStore (interaction_dag.go:66) + MessageStore (:70); construction failure slog.Error + nil (diagnosis is reward-quality not correctness - a silent nil would let rollouts run reward-less undetected; Error makes the misconfig visible without crashing the daemon); nil diagnoser safe (maybeDiagnoseProject gates on deps.Diagnosis==nil). DEVIATION from plan (justified): env lives in training_config.go NOT daemon/config.go - config.go:261 MULTICA_PI_PATH probing is dispatch-agent CLI discovery (a separate concern); training_config.go is where InteractionDAGEnabled + LoadTrainingConfig + NewTrainingSessionDeps already live (the direct analog). Test file is training_config_test.go (not config_test.go) for the same reason. GAP-CLOSURE: commit 5ed9f358a (rich-prompt Diagnose, made between Task 4 and Task 6) CLOSED Task 4 deferred gap (1) "Diagnose placeholder prompt" - Diagnose now fetches GetInteractionDAG+GetSegmentMessages+GetTaskContext and embeds them as a JSON payload in the prompt with --no-tools (mirrors evolution_review_provider.go); openspec 3.4 (views whole DAG) now satisfied. Controller-verified: 5 new tests PASS (TestLoadTrainingConfig_DiagnosisDisabledByDefault/Enabled/InvalidTimeoutAndScoreMax + TestNewTrainingSessionDeps_DiagnosisWiredWhenEnabled/DiagnosisNilWhenBridgeConfigMissing); gofmt/vet/build clean across service+handler+daemon+cmd/server; full service suite green. Production chain confirmed: cmd/server/main.go:335-336 LoadTrainingConfig->NewTrainingSessionDeps->WithTraining. REMAINING deferred: (2) strict 4.4 /dag 202-during-diagnosis (needs an in-progress flag); (3) Task 4 test-strengthening (RTT-level ordering) -> Task 9 integration (7.1 exercises the real RTT path).
- [x] Task 7: AReal consumer - step_rewards -> SuperNode.process_reward (Python + Go) - COMPLETE (areal worktree 7bbb7d52 + multica dev 34526d9a4; areal worktree MERGED origin/master e07517ca - 0 behind, 9 ahead). Orchestrator-IMPLEMENTED DIRECTLY (TDD RED-first). D4 RECONCILIATION (user-approved option 1): design said map (segment_id,seq)->per-turn Node.process_reward, but v2 production path (assemble_from_refs->events_from_nodes) builds SuperNodes w/ nodes=[] and GAE consumes per-segment SuperNode.process_reward (events_from_nodes gae.py:135 reads super_node.process_reward; NOT per-turn Nodes). So step_rewards are AGGREGATED to a per-segment reward: mean(scores)/score_max in [0,1] (the per-segment GAE reward; per-turn scores remain stored in multica interaction_dag_step_reward + /dag for future per-token use). Go half (34526d9a4): AssembledDag.ScoreMax int (json:score_max) - serving-boundary metadata, AssembleAssembledDag leaves 0, GetDag handler stamps dag.ScoreMax=LoadTrainingConfig().DiagnosisAgentScoreMax before serving (same env source as the runner; 0=not configured->sparse); footgun-free (areal doesn't guess multica's scale - boundary canonicalization); TestGetDag asserts score_max served. Python half (7bbb7d52): StepReward dataclass + AssembledDag.step_rewards + score_max parsed in from_dict (absent->empty list + 0, never fabricated); supernode_assembler.assemble_from_refs indexes step_rewards by segment_id, drops+logs unmatched (no spurious SuperNode, never fatal), sets SuperNode.process_reward=_aggregate_process_reward(scores, score_max) (0.0 when unscored or score_max==0 -> sparse, matches old enable_judge_process_reward=False fallback). Controller-verified: 5 new tests PASS (test_assembled_dag_from_dict_parses_step_rewards_and_score_max + _defaults + test_assemble_from_refs_aggregates_step_rewards_to_process_reward + _score_max_zero_is_sparse + _drops_unmatched_step_rewards); 379 pass in full tree_search suite; ruff clean; 10 pre-existing failures (datasets missing, asyncio event-loop) confirmed identical on merged-master baseline via stash - unrelated. DEVIATION from plan (justified): aggregate to SuperNode.process_reward NOT per-turn Node.process_reward (D4 - v2 GAE is per-segment); score_max served in /dag NOT areal-config (boundary canonicalization). REMAINING deferred: (2) strict 4.4 /dag 202-during-diagnosis; (3) Task 4 RTT-level ordering test -> Task 9 integration.
- [x] Task 8: Remove the flat areal judge (Python) - COMPLETE (areal worktree f85cbf11, minimal-safe user-approved). Orchestrator-IMPLEMENTED DIRECTLY. Removed the opt-in flat-LLM-judge process-reward path from customized_grouped_workflow.py so process_reward comes solely from Task 7 (SuperNode.process_reward <- step_rewards, consumed by GAE). Removed: _annotate_judge_process_rewards method + _backup_fresh_with_returns (judge-only deferred backup) + _judged_episodes cache; 4 ctor params (enable_judge_process_reward/judge_process_reward_beta/judge_model_name/judge_max_concurrency) + self assignments; 3 judge_beta=(...) conditionals on the advantage computers (judge_beta defaults 0.0 -> no judge shaping; SuperNode.process_reward is the GAE reward); defer_backup logic + judge annotation call site + deferred-backup block (insert-time backup restored backup=True, the pre-judge default). config.py: 4 judge fields + __post_init__ validation + comment. distilling/config.py: judge_model_name/judge_base_url/judge_api_key (not read by any kept code - ExternalDiagnoseProvider uses diagnose_* params). Deleted tests: test_judge_annotation/test_judge_integration/test_config_judge (tested removed path). KEPT (deviation from plan's 'delete judge_prompt.py', justified): judge_prompt.py/diagnose_provider.py/ExternalDiagnoseProvider/DiagnoseProvider protocol/selected_turn_distill/test_judge_provider/test_judge_prompt - the distilling infrastructure; diagnose_episode uses judge_prompt's build_judge_instruction/parse_turn_scores, and selected_turn_distill depends on the DiagnoseProvider protocol which REQUIRES diagnose_episode - plan's 'delete judge_prompt.py' + 'keep diagnose_episode' were contradictory, minimal-safe keeps distilling utilities intact (now unused by training workflow, available for distillation). Controller-verified: imports clean; judge ctor params gone; 364 pass in full tree_search suite (15 fewer = 3 deleted test files); 10 pre-existing failures (datasets+asyncio) unchanged from baseline; kept distilling tests (test_judge_provider + test_judge_prompt) 12 pass; ruff format clean (5 pre-existing StrEnum suggestions unrelated). NEXT: Task 9 (integration + E2E + sweep; resolve design.md Q1-Q4; check off tasks.md). NOTE: Task 9 integration/E2E need live multica+areal+GPU (CLAUDE.md: integration tests require multi-node hardware) - will run what's runnable, skip+explain the rest.
- [x] Task 9: Integration + E2E + sweep + design finalization (RUNNABLE PORTION COMPLETE; areal worktree 26037ab2). Orchestrator-IMPLEMENTED DIRECTLY. Done: (8.1) grep sweep - production code clean (judge gone from tree_search/*.py; step_rewards/start_seq/end_seq resolve to Task 2/5/7 code; remaining judge_model_name refs are tpfc/ separate subsystem + README stale docs deferred); (8.2) design.md D4 updated to Task 7 reconciliation (aggregate to per-segment SuperNode.process_reward = mean(scores)/score_max, NOT per-turn Node - v2 GAE is per-segment) + D5 corrected (CompleteAgentTask persists terminal before RouteTerminalTrainingTask -> /dag is 200 not 202 during synchronous diagnosis; strict 202 deferred); (6.2) spec diagnosis-process-reward/spec.md L68 clarified to per-segment SuperNode attachment; (Step 6) tasks.md 15 done items checked off (3.4, 4.5-4.6, 5.1-5.6, 6.1-6.3, 8.1-8.3). Regression: Go internal/service suite green + handler GetDag/diagnosis tests pass (handler AgentActivity/Credential/Transport failures are pre-existing DB 500s in untouched files); areal tree_search suite 364 pass (16 new tests Tasks 7-8). DEFERRED to a GPU hardware verify run: 7.1 integration (trained 3-agent rollout -> diagnosis -> step_rewards -> /dag), 7.2 E2E (rollout -> areal resolve -> ExecutionDag process_reward -> training step), 7.3 sanity (diagnosis vs removed judge - judge deleted, can't run), 7.4 commit; 4.4 strict /dag 202-during-diagnosis (needs in-progress flag). BUILD COMPLETE at unit level; live integration/E2E await a hardware environment.
