...
**Input**: The argument after `/opsx-propose` is the change name (kebab-case), OR a
description of what the user wants to build. **Provided arguments**: $@
...
1. **Create artifacts in sequence until apply-ready**
   ...
   Loop through artifacts in dependency order (artifacts with no pending dependencies
   first):
...
- If a change with that name already exists, ask if user wants to continue it or create
  a new one
